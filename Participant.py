...
class Computer(Participant):
    ...
    def almostStraight(self, table):
        ...
        if (possible or improbable) and not actuallyThereIs:
            ...
            total = [each.number for each in self.getCards()] + numbers
        ...
        if actuallyThereIs:
            ...
            hand = [each.numbers for each in self.getCards()]
    ...
    def almostFlush(self, table):
        ...
        add = [each.suit for each in self.getCards()]
        total = suits.copy()
        for each in add:
            if each in total:
                total[each] += 1
    ...
    def main(self, table, player):
        if table.cards == None:
            winPerc = self.winningStatistic()
            notFear = winPerc
            fear = notFear - 30
            ...
            result1, card1 = self.almostStraight(table)
            result2, card2 = self.almostFlush(table)
            result3, card3 = self.almostPoker(table)
        ...
        if action == 'Bet' and player.bet not in [None, 0]:
            ...
            available = [
                i for i in range(self.minBet, self.maxBet + 1, table.blind)
            ]
