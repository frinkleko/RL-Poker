...
def almostFlush(hand, table):
    ...
    if 5 in suits.values():
        ...
        if flushSuit in add:
            ...
            maxHand = max(
                [card.number for card in hand.cards if card.suit == flushSuit])
            if maxTable < maxHand:
                return "Flush on table and max hand", max([
                    card.number for card in hand.cards
...
# BET
    ...
    if action == 'Bet' and player.bet not in [None, 0]:
        ...
        available = [
            i for i in range(computer.minBet, computer.maxBet + 1, table.blind)
        ]
